...
async function connectToWhatsApp() {
  ...
  console.log(
    color(`${spc2}           [ Created by: Zyko MD ]`, `${randomcolor}`),
  );
  console.log(
    color(
      `${spc4}                       < =============================== >`,
  ...
  console.log(
    color(`${spc3}                    [•]`, `${randomcolor}`),
    color(`Hai         : i.m ${namebot}`, `${randomcolor}`),
  );
  console.log(
    color(`${spc3}                    [•]`, `${randomcolor}`),
    color(`Bot Version : 1.0.0`, `${randomcolor}`),
  );
  console.log(
    color(`${spc3}                    [•]`, `${randomcolor}`),
    color(`Library     : Baileys Multi Device`, `${randomcolor}`),
  );
  console.log(
    color(`${spc3}                    [•]`, `${randomcolor}`),
    color(`Status      : Online!`, `${randomcolor}`),
  );
  console.log(
    color(`${spc3}                    [•]`, `${randomcolor}`),
    color(`Owner       : ${namaowner}`, `${randomcolor}`),
  );
  console.log(
    color(`${spc3}                    [•]`, `${randomcolor}`),
    color(`Author      : Zyko MD`, `${randomcolor}`),
  );
  console.log(
    color(`${spc3}                    [•]`, `${randomcolor}`),
    color(`Base Ori By : Zyko MD`, `${randomcolor}`),
  );
  console.log(
    color(
      `${spc4}                       < =============================== >`,
  ...
  if (usePairingCode && !zyko.authState.creds.registered) {
    const phoneNumber = await question(
      "Masukan Nomer Yang Aktif Awali Dengan 62 Recode : zykomdofficial:\n",
    );
    ...
    try {
      ...
      mek.message =
        Object.keys(mek.message)[0] === "ephemeralMessage"
          ...
          : mek.message;
      if (mek.key && mek.key.remoteKid === "status@broadcast" && setting.autosw)
        setTimeout(() => {
          conn.readMessage([mek.key]);
        }, 2000);
      if (!zyko.public && !mek.key.fromMe && chatUpdate.type === "notify")
        return;
    ...
    for (let kopel of celled) {
      if (kopel.isGroup == false) {
        if (kopel.status == "offer") {
          let nomer = await zyko.sendTextWithMentions(
            kopel.from,
            `*${zyko.user.name}* tidak bisa menerima panggilan ${kopel.isVideo ? `video` : `suara`}. Maaf @${kopel.from.split("@")[0]} kamu akan diblokir. Silahkan hubungi Owner membuka blok !`,
          );
          zyko.sendContact(
            kopel.from,
...
`,
          ...
          zyko.sendMessage(anu.id, {
            image: { url: ppuser },
            mentions: [num],
            caption: `Selamat Tinggal *@${num.split("@")[0]}* Di Group *${metadata.subject}*`,
          });
          ...
          zyko.sendMessage(anu.id, {
            image: { url: ppuser },
            mentions: [num],
            caption: `@${num.split("@")[0]} Ciee Di Hapus Jadi Admin Di Group ${metadata.subject}\n\n${metadata.desc}`,
          });
        }
      }
    } catch (err) {
  ...
  zyko.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      zyko.sendMessage(global.owner + "@s.whatsapp.net", {
  ...
  //=================================================//
  zyko.sendText = (jid, text, quoted = "", options) =>
    zyko.sendMessage(jid, { text: text, ...options }, { quoted });
  //=================================================//
  zyko.sendTextWithMentions = async (jid, text, quoted, options = {}) =>
    zyko.sendMessage(
      jid,
      {
        text: text,
  ...
  //=================================================//
  zyko.downloadAndSaveMediaMessage = async (
    message,
    filename,
...
return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net')}*/
  ...
  zyko.copyNForward = async (
    ...
    await zyko.relayMessage(jid, waMessage.message, {
  ...
  zyko.sendMessageV2 = async (chatId, message, options = {}) => {
    ...
    return await zyko.relayMessage(chatId, generate.message, {
    ...
    for (const message of m) {
      try {
        ...
        if (
          ...
          zyko.processMessageStubType(message).catch(console.error);
        ...
        if (isGroup) {
          if (!chats.subject || !chats.metadata) {
            metadata =
              (await zyko.groupMetadata(chat).catch((_) => ({}))) || {};
            if (!chats.subject) chats.subject = metadata.subject || "";
            if (!chats.metadata) chats.metadata = metadata;
          }
          sender = zyko.decodeJid(
            (message.key?.fromMe && zyko.user.id) ||
              message.participant ||
        ...
        const fromMe =
          message.key.fromMe || areJidsSameUser(sender || chat, zyko.user.id);
        if (
