...
module.exports = zyko = async (zyko, m, chatUpdate, store) => {
  ...
  try {
    ...
    const groupAdmins = m.isGroup
      ? await participants.filter((v) => v.admin !== null).map((x) => x.id)
      : "";
    const groupOwner = m.isGroup ? groupMetadata.owner : "";
    const isBotAdmins = m.isGroup ? groupAdmins.includes(botNumber) : false;
    const isAdmins = m.isGroup ? groupAdmins.includes(m.sender) : false;
    const isGroupAdmins = m.isGroup ? groupAdmins.includes(m.sender) : false;
    const isBotGroupAdmins = m.isGroup
      ? groupAdmins.includes(botNumber)
      : false;
    const sender = m.key.fromMe
      ? zyko.user.id.split(":")[0] + "@s.whatsapp.net" || zyko.user.id
    ...
    if (db.data.settings[botNumber].autobio) {
      ...
      if (new Date() * 1 - setting.status > 1000) {
        ...
        await zyko
          .updateProfileStatus(
            `I am ${namabot} | Aktif Selama ${uptime}| Mode : ${zyko.public ? "Public-Mode" : "Self-Mode"} | User : ${Object.keys(global.db.data.users).length}`,
          )
          .catch((_) => _);
    ...
    const reply = (teks) => {
      return zyko.sendMessage(
        from,
        {
          text: teks,
          contextInfo: {
            externalAdReply: {
              title: `Developer ${namabot}`,
              body: `Selamat ${waktuucapan} kak ${pushname}`,
              previewType: "PHOTO",
              thumbnailUrl: "https://telegra.ph/file/46ef83ea6db7881b66cf7.jpg",
              thumbnail: thumb,
              sourceUrl: sgc,
            },
          },
    ...
    if (db.data.chats[m.chat].antilink2) {
      if (budy.match(`chat.whatsapp.com`)) {
        ...
        let gclink =
          `https://chat.whatsapp.com/` + (await zyko.groupInviteCode(m.chat));
    ...
    if (!isCmd && isGroup && isAlreadyResponList(from, body, db_respon_list)) {
      ...
      if (get_data_respon.isImage === false) {
        zyko.sendMessage(
          from,
          { text: sendResponList(from, body, db_respon_list) },
          {
            quoted: m,
          },
        );
...
+1 Pangkat Yang Terbuka`.trim();
    ...
    const cek = (satu, dua) => {
      ...
      if (zyko !== false) {
        if (satu == "id") {
          return member[zyko].id;
        }
        if (satu == "saldo") {
          return member[zyko].saldo;
        }
        if (satu == "transaksi") {
          return member[zyko].transaksi;
        }
        if (satu == "idproduk") {
          return member[zyko].idproduk;
        }
        if (satu == "idtujuan") {
          return member[zyko].idtujuan;
        }
        if (satu == "nama") {
          return member[zyko].nama;
        }
        if (satu == "harga") {
          return member[zyko].harga;
        }
        if (satu == "seri") {
          return member[zyko].seri;
        }
      }
      if (zyko == false) {
        return null;
    ...
    switch (command) {
      case "poll":
        {
          if (!text)
            return m.reply(
              `Contoh: ${prefix + command} Besok main gk?|Ayo aja|Ga ah malas`,
            );
          let anu = text.split("|");
          let name = anu.slice(0, 1);
          let value = anu.slice(1);
          zyko.sendMessage(m.chat, {
            poll: { name: name, values: value, selectableCount: 1 },
          });
        }
        break;
      case "listsw":
        {
          if (zyko.messages["status@broadcast"].array.length === 0)
          ...
          story.forEach((obj) => {
            ...
            participant = jidNormalizedUser(
              participant === "status_me" ? zyko.user.id : participant,
            );
      ...
      case "upsw":
        if (!m.isOwner) return;
        let statusJidList = Object.values(store.contacts)
      ...
      case "regis":
        {
          ...
          if (m.isGroup) {
            m.reply(
              `🇮🇩 _Bot telah mengirimkan pendaftaran ke private chat, silahkan selesaikan agar dapat menggunakan fitur bot ${namabot}._`,
            );
            if (!m2)
              return zyko.sendMessage(
                sender,
                {
                  text: `🇮🇩 _Hi @${sender.split("@")[0]} silahkan ketik umur mu .daftar nama|usia\n\ncontoh .daftar zyko|18`,
          ...
          if (!m3)
            return zyko.sendMessage(
              sender,
              {
                text: `🇮🇩 _Hi @${sender.split("@")[0]} silahkan pilih jenis kelamin kamu dengan cara ketik nama|usia|jeniskelamin\n\ncontoh .daftar zyko|18|pria`,
                mentions: [`${sender.split("@")[0]}@s.whatsapp.net`],
              },
              { quoted: m },
            );
      ...
      case "del":
        {
          ...
          if (!isBaileys)
            return reply("Pesan tersebut bukan dikirim oleh bot!");
          zyko.sendMessage(m.chat, {
            delete: {
              remoteJid: m.chat,
              fromMe: true,
      ...
      case "sholawat":
        {
          ...
          zyko.sendMessage(
            m.chat,
            { audio: rvn, mimetype: "audio/mp4", ptt: true },
            { quoted: m },
          );
        }
        break;
      ...
      case "alquran":
        {
          ...
          try {
            let f = await fetch(
              `https://api.alquran.cloud/v1/surah/${text}/ar.alafasy`,
            );
            let xx = await f.json();
...
Artinya: "Sesungguhnya Allah mempunyai sembilan puluh sembilan nama, alias seratus kurang satu. Barang siapa yang menghitung-hitungnya, niscaya masuk surga; Dia Witir dan menyukai yang witir".`;
          ...
          if (args[0]) {
            if (args[0] < 1 || args[0] > 99)
              return reply(`minimal 1 & maksimal 99!`);
...
`;
      ...
      case "viral":
        {
          ...
          await sleep(4000);
          m.reply(
            "🚩 successfully sent to private chat, please check the botz message",
          );
        }
        break;
      case "yuri":
      ...
      case "gunung":
        {
          let g = await fetch(
            `https://raw.githubusercontent.com/inirey/RESTAPI/master/data/Mountain.json`,
          );
          let f = await g.json();
      ...
      case "hacker":
        {
          let g = await fetch(
            `https://raw.githubusercontent.com/inirey/RESTAPI/master/data/hekel.json`,
          );
          let f = await g.json();
          let a = f[Math.floor(Math.random() * f.length)];
      ...
      case "wptechnology":
        {
          let g = await fetch(
            `https://raw.githubusercontent.com/inirey/RESTAPI/master/data/Technology.json`,
          );
          let f = await g.json();
          let a = f[Math.floor(Math.random() * f.length)];
      ...
      case "wpaesthetic":
        {
          let g = await fetch(
            `https://raw.githubusercontent.com/inirey/RESTAPI/master/data/aesthetic.json`,
          );
          let f = await g.json();
          let a = f[Math.floor(Math.random() * f.length)];
      ...
      case "wpgame":
        {
          let g = await fetch(
            `https://raw.githubusercontent.com/inirey/RESTAPI/master/data/GameWallp.json`,
          );
          let f = await g.json();
          let a = f[Math.floor(Math.random() * f.length)];
...
`;
      ...
      case "nguli":
        {
          ...
          if (
            new Date() - global.db.data.users[m.sender].lastnguli >
            86400000
          ) {
      ...
      case "mulung":
        {
          ...
          if (new Date() - global.db.data.users[m.sender].lastmulung < 1800000)
            return `Anda sudah lelah untuk mulung\nTunggu selama ${msToTime(time - new Date())} lagi`;
      ...
      case "monthly":
        {
          ...
          if (new Date() - user.lastmonthly < cooldownn)
            return reply(
              `You have already claimed this monthly claim, wait for *${(user.lastmonthly + cooldownn - new Date()).toTimeString()}*`,
            );
          let text = "";
          for (let reward of Object.keys(rewardss))
            if (reward in user) {
      ...
      case "premgift":
        {
          if (args.length == 0)
            return zyko.sendText(m.chat, `Harap masukan Kode FreeGiftmu!`, m);
          if (
            args[0] == "OshdnpGaka" ||
            args[0] == "PagpqvUac" ||
            args[0] == "91hakHcwo" ||
            ...
            if (
              new Date() - global.db.data.users[m.sender].lastgift >
              86400000
            ) {
              zyko.sendText(
                m.chat,
              ...
              zyko.sendText(
                m.chat,
                "[❗] Kode Gift Gratis hanya dapat digunakan sehari sekali !\n\nKetik *!buygift* untuk membeli kodegift premium",
                m,
              );
          } else {
            zyko.sendText(
      ...
      case "weekly":
        {
          ...
          if (new Date() - user.lastweekly < cooldown)
            return reply(
              `You have already claimed this daily claim!, wait for *${(user.lastweekly + cooldown - new Date()).toTimeString()}*`,
            );
          let text = "";
...
`.trim();
          ...
          if (command.toLowerCase() == "buy") {
            ...
            if (user[paymentMethod] < listItems[item][paymentMethod] * total)
              return m.reply(
                `You don't have enough ${global.rpg.emoticon(paymentMethod)}${paymentMethod} to buy *${total}* ${global.rpg.emoticon(item)}${item}. You need *${listItems[item][paymentMethod] * total - user[paymentMethod]}* more ${paymentMethod} to be able to buy`,
              );
            ...
            if (user[item] < total)
              return m.reply(
                `You don't have enough *${global.rpg.emoticon(item)}${item}* to sell, you only have ${user[item]} items`,
              );
            user[item] -= total;
      ...
      case "petualang":
        {
          if (!m.isGroup) return reply(mess.group);
          let __timers =
            new Date() - global.db.data.users[m.sender].lastadventure;
            ...
            m.chat,
            {
              ...
              contextInfo: {
                externalAdReply: {
                  title: v,
                  thumbnailUrl:
                    "https://telegra.ph/file/00ee61b555099bbfdd614.jpg",
                  mediaType: 1,
                  renderLargerThumbnail: true,
                },
              },
      ...
      case "nabung":
        {
          ...
          if (user.bank > user.fullatm)
            return m.reply("Uang dibankmu sudah penuh!");
...
`;
      ...
      case "berkebun":
        {
          ...
          if (
            new Date() - global.db.data.users[m.sender].lastberkebon <
            1800000
          )
            return reply(
              ...
              if (global.db.data.users[m.sender].bibitjeruk > 499) {
                if (global.db.data.users[m.sender].bibitmangga > 499) {
                  if (global.db.data.users[m.sender].bibitpisang > 499) {
                    ...
                    let anggurpoin =
                      `${Math.floor(Math.random() * 500)}`.trim();
                    ...
                    global.db.data.users[m.sender].lastberkebon =
                      new Date() * 1;
                    reply(
                      `Selamat kamu mendapatkan :\n\n+${apelpoin} Apel\n+${anggurpoin} Anggur\n+${jerukpoin} Jeruk\n+${manggapoin} Mangga\n+${pisangpoin} Pisang\n+1 Tiketcoin`,
                    );
                    ...
                    reply(
                      `Pastikan bibit anggur kamu *500* untuk bisa berkebun`,
                    );
                } else
                  reply(`Pastikan bibit apel kamu *500* untuk bisa berkebun`);
              } else
                reply(`Pastikan bibit jeruk kamu *500* untuk bisa berkebun`);
            } else
              reply(`Pastikan bibit mangga kamu *500* untuk bisa berkebun`);
      ...
      case "bonus":
        {
          if (
            new Date() - global.db.data.users[m.sender].lastclaim >
            86400000
          ) {
      ...
      case "merampok":
        {
          ...
          if (
            new Date() - global.db.data.users[m.sender].lastmulung <
            604800000
          )
            return reply(
              `*Kamu sudah merampok!*\n\nTunggu selama ${msToTime(time - new Date())} lagi`,
            ...
            m.chat,
            {
              ...
              contextInfo: {
                externalAdReply: {
                  title: v,
                  thumbnailUrl:
                    "https://telegra.ph/file/a0d4dadf97dc29fba2c32.jpg",
                  sourceUrl: sgc,
                  mediaType: 1,
                  renderLargerThumbnail: true,
                },
      ...
      case "bug":
        {
          ...
          if (m.isGroup) {
            m.reply(
              `_Pesan terkirim kepemilik bot, jika ${command} hanya main-main tidak akan ditanggapi._`,
            );
          }
        }
        break;
...
*"ᴏʀᴀɴɢ ʏᴀɴɢ ᴍᴇɴɢᴜᴄᴀᴘᴋᴀɴ ꜱᴀʟᴀᴍ ꜱᴇᴘᴇʀᴛɪ ɪɴɪ ᴍᴀᴋᴀ ɪᴀ ᴍᴇɴᴅᴀᴘᴀᴛᴋᴀɴ 30 ᴘᴀʜᴀʟᴀ, ᴋᴇᴍᴜᴅɪᴀɴ, ᴏʀᴀɴɢ ʏᴀɴɢ ᴅɪʜᴀᴅᴀᴘᴀɴ ᴀᴛᴀᴜ ᴍᴇɴᴅᴇɴɢᴀʀɴʏᴀ ᴍᴇᴍʙᴀʟᴀꜱ ᴅᴇɴɢᴀɴ ᴋᴀʟɪᴍᴀᴛ ʏᴀɴɢ ꜱᴀᴍᴀ ʏᴀɪᴛᴜ “ᴡᴀ'ᴀʟᴀɪᴋᴜᴍ ꜱᴀʟᴀᴍ ᴡᴀʀᴀʜᴍᴀᴛᴜʟʟᴀʜɪ ᴡᴀʙᴀʀᴀᴋᴀᴛᴜʜ” ᴀᴛᴀᴜ ᴅɪᴛᴀᴍʙᴀʜ ᴅᴇɴɢᴀɴ ʏᴀɴɢ ʟᴀɪɴ (ᴡᴀʀɪᴅʜᴡᴀᴀɴᴀ). ᴀʀᴛɪɴʏᴀ ꜱᴇʟᴀɪɴ ᴅᴀʀɪᴘᴀᴅᴀ ᴅᴏ'ᴀ ꜱᴇʟᴀᴍᴀᴛ ᴊᴜɢᴀ ᴍᴇᴍɪɴᴛᴀ ᴘᴀᴅᴀ ᴀʟʟᴀʜ ꜱᴡᴛ"* `;
          ...
          reply(info, m, {
            contextInfo: {
              externalAdReply: {
                title: wm,
                body: "ᴡᴀᴀʟᴀɪᴋᴜᴍᴍᴜꜱꜱᴀʟᴀᴍ ᴡᴀʀᴀʜᴍᴀᴛᴜʟʟᴀʜɪ ᴡᴀʙᴀʀᴏᴋᴀᴛᴜʜ",
                sourceUrl: owner,
                thumb: thumb,
              },
            },
              ...
              m.chat,
              {
                ...
                contextInfo: {
                  externalAdReply: {
                    title: v,
                    thumbnailUrl:
                      "https://telegra.ph/file/975b07343239240e5a0be.jpg",
                    mediaType: 1,
                    renderLargerThumbnail: true,
                  },
                },
      ...
      case "tebakangka":
        {
          ...
          if (!args[0])
            return zyko.sendText(m.chat, `Harap masukkan angka tebakanmu`, m);
          if (args[0] < 1 || args[0] > 100)
            return zyko.sendText(
              m.chat,
              `Harap masukkan angka antara 1 dan 100`,
              m,
            );
      ...
      case "kuismath":
        {
          if (kuismath[m.chat])
            return zyko.sendMessage(
              m.chat,
              { text: "Soal ini belum selesai" },
              { quoted: kuismath[m.chat][0] },
            );
          let { genMath, modes } = require("./lib/math");
          if (!text)
            return reply(
              `*Example: ${prefix + command} noob*\n\nMode : ${Object.keys(modes).join(" | ")}`.trim(),
            );
      ...
      case "tebaklagu":
        {
          if (tebaklagu[m.chat])
            return zyko.sendMessage(
              m.chat,
              { text: "Soal ini belum selesai" },
              { quoted: tebaklagu[m.chat][0] },
            );
          let poin = 1000;
          let src = await fetchJson(
            "https://raw.githubusercontent.com/Aiinne/scrape/main/tebaklagu.json",
          );
          let json = src[Math.floor(Math.random() * src.length)];
          let msg = await zyko.sendMessage(
            m.chat,
            { audio: { url: json.lagu }, mimetype: "audio/mpeg" },
            { quoted: m },
          );
...
*Balas pesan ini untuk Menjawab atau Menyerah!*`),
      ...
      case "selengkapnya":
        {
          if (!q)
            return reply(
              `masukan text contoh ${command} kamujelek|tapii boong`,
            );
          let [l, r] = text.split`|`;
          if (!l) l = "";
          if (!r) r = "";
      ...
      case "createqr":
        {
          if (!isPremium && global.db.data.users[m.sender].limit < 2)
            return reply(mess.endLimit);
          ...
          if (!text)
            return reply(
              `Penggunaan Salah Harusnya ${prefix + command} zykomd`,
            );
      ...
      case "gtts":
        {
          let sp = "•";
          if (!text.includes(" "))
            return reply(`*Example : ${prefix + command} kodebahasa text*\n\n${sp} af: Afrikaans,
${sp} sq: Albanian
${sp} ar: Arabic
${sp} hy: Armenian
${sp} ca: Catalan
      ...
      case "get":
        {
          ...
          if (!/text|json/.test(res.headers.get("content-type")))
            return zyko.sendFile(m.chat, url, waktuucapan, author, m, null, {
      ...
      case "jarak":
        {
          ...
          if (!(text1 && text2))
            return reply(`🚩 *Example:* ${prefix + command} Sukabumi|bandung`);
      ...
      case "lirik":
        {
          ...
          const result = await lyricsv2(teks).catch(
            async (_) => await lyrics(teks),
          );
          m.reply(
            `
Lyrics *${result.title}*
      ...
      case "translate":
        {
          ...
          if (!res)
            return reply(`*Error : The language "${lang}" is not supported*`);
          let gtts = require("./lib/scraper/gtts")(`${lang}`, `${res.text}`);
          let ranm = getRandom(".mp3");
          let rano = getRandom(".ogg");
          res.text.length > 10000
            ? reply("Teks nya terlalu panjang")
            : gtts.save(ranm, res.text, function () {
                exec(
                  `ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`,
                  (err) => {
                    fs.unlinkSync(ranm);
      ...
      case "pinterest":
        {
          ...
          reply(mess.wait);
          if (!isPremium && global.db.data.users[m.sender].limit < 1)
            return reply(mess.endLimit); // respon ketika limit habis
      ...
      case "coffee":
        {
          ...
          if (json.file) {
            zyko.sendMessage(
              m.chat,
              { image: { url: json.file }, caption: "Ngopi skuy" },
              { quoted: m },
            );
          } else {
            return reply(json);
      ...
      case "meme":
        {
          ...
          zyko.sendMessage(
            m.chat,
            { image: { url: url.image }, caption: "Nih Memeindo" },
            { quoted: m },
          );
        }
        break;
      case "githubsearch":
        {
          ...
          let res = await fetch(
            `https://api.github.com/search/repositories?q=${encodeURIComponent(text)}`,
          );
      ...
      case "weather":
        {
          ...
          let res = await fetch(
            `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(text)}&units=metric&appid=060a6bcfa19809c2cd4d97a212b19273`,
          );
      ...
      case "cecanukhty":
        {
          if (!isPremium && global.db.data.users[m.sender].limit < 1)
            return reply(mess.endLimit);
          db.data.users[m.sender].limit -= 1;
          let bpink = await fetch(
            "https://raw.githubusercontent.com/arivpn/dbase/master/kpop/ukhty.txt",
          )
            .then((res) => res.text())
            .then((txt) => txt.split("\n"));
          ...
          zyko.sendMessage(
            m.chat,
            {
              image: { url: img },
              caption:
                "© Nih cecanukhty, jangan lupa subscribe channel SunShine 👍",
            },
            { quoted: m },
          );
        }
      ...
      case "exo":
        {
          ...
          db.data.users[m.sender].limit -= 1;
          let bpink = await fetch(
            "https://raw.githubusercontent.com/arivpn/dbase/master/kpop/exo.txt",
          )
            .then((res) => res.text())
            .then((txt) => txt.split("\n"));
          let img = bpink[Math.floor(Math.random() * bpink.length)];
          ...
          zyko.sendMessage(
            ...
            {
              image: { url: img },
              caption: "© Nih exo, jangan lupa subscribe channel SunShine 👍",
            },
            { quoted: m },
          );
        }
      ...
      case "blackpink":
        {
          ...
          db.data.users[m.sender].limit -= 1;
          let bpink = await fetch(
            "https://raw.githubusercontent.com/arivpn/dbase/master/kpop/blekping.txt",
          )
            .then((res) => res.text())
            .then((txt) => txt.split("\n"));
          let img = bpink[Math.floor(Math.random() * bpink.length)];
          ...
          zyko.sendMessage(
            m.chat,
            {
              image: { url: img },
              caption:
                "© Nih Blackpinknya, jangan lupa subscribe channel SunShine 👍",
            },
            { quoted: m },
          );
        }
      ...
      case "bts":
        {
          ...
          db.data.users[m.sender].limit -= 1;
          let bts = await fetch(
            "https://raw.githubusercontent.com/arivpn/dbase/master/kpop/batues.txt",
          )
            .then((res) => res.text())
            .then((txt) => txt.split("\n"));
          ...
          zyko.sendMessage(
            ...
            {
              image: { url: img },
              caption: "© Nih bts, jangan lupa subscribe channel SunShine 👍",
            },
            { quoted: m },
          );
        }
      ...
      case "cecanrandom":
        {
          ...
          db.data.users[m.sender].limit -= 1;
          let arr = await fetch(
            "https://raw.githubusercontent.com/arivpn/dbase/master/kpop/cecan.txt",
          )
            .then((res) => res.text())
            .then((txt) => txt.split("\n"));
          ...
          zyko.sendMessage(
            m.chat,
            {
              image: { url: img },
              caption:
                "© Nih cecanrandom, jangan lupa subscribe channel SunShine 👍",
            },
            { quoted: m },
          );
        }
      ...
      case "asupan":
        {
          ...
          zyko.sendMessage(
            sender,
            { video: { url: pickRandom(global.asupan) }, caption: wm },
            { quoted: m },
          );
        }
        break;
      ...
      case "nomorhoki":
        {
          ...
          db.data.users[m.sender].limit -= 1;
          if (!Number(text))
            return reply(`Contoh : ${prefix + command} 6288292024190`);
      ...
      case "ramaljodoh":
        {
          ...
          if (!text)
            return reply(
              `Contoh : ${prefix + command} Dika, 7, 7, 2005, Novia, 16, 11, 2004`,
            );
          let [nama1, tgl1, bln1, thn1, nama2, tgl2, bln2, thn2] =
            text.split`,`;
      ...
      case "apkdl":
        {
          ...
          if (!q)
            return reply(
              `Ex: ${prefix + command} https://play.google.com/store/apps/details?id=com.linecorp.LGGRTHN`,
            );
      ...
      case "fbdl":
        {
          ...
          if (!text)
            return reply(
              `Example : ${prefix + command} https://www.facebook.com/100065164892842/posts/731701032345387/?mibextid=rS40aB7S9Ucbxw6v`,
            );
      ...
      case "ig":
        {
          if (!args[0])
            return reply(
              `🚩 *Example:* ${prefix + command} https://www.instagram.com/reel/Co18PSBAmkh/?igshid=MmJiY2I4NDBkZg==`,
            );
          const { status, msg, data } = await FongsiDev_Scraper.Instagram(
            args[0],
          );
          if (status !== 200) reply(msg);
          zyko.sendMessage(
            m.chat,
            {
      ...
      case "mf":
        {
          ...
          if (!/https?:\/\/(www\.)?mediafire\.com/.test(args[0]))
      ...
      case "youtubemp3":
        {
          ...
          if (!text)
            return reply(
              `Example : ${prefix + command} https://youtube.com/shorts/Akm3ijSj5Xc?si=2bWdhUa4dhAxLiV0`,
            );
      ...
      case "getmusic":
        {
          ...
          if (!m.quoted.isBaileys)
            return reply(`Hanya Bisa Membalas Pesan Dari Bot`);
          ...
          if (!urls)
            return reply(
              `Mungkin pesan yang anda reply tidak mengandung result ytsearch`,
            );
          await reply(mess.wait);
      ...
      case "tt":
        {
          ...
          if (!q)
            return reply(
              `🚩 *Example:* ${prefix + command} https://vt.tiktok.com/ZSNCXnAUj/`,
            );
          const { status, msg, data } = await FongsiDev_Scraper.TiktokVideo(
            args[0],
          );
          if (status !== 200) reply(msg);
...
`;
          if (data?.images) {
            ...
            zyko.sendMessage(
              m.chat,
              { video: { url: data.play }, caption: cap },
              { quoted: m },
            );
          }
        }
      ...
      case "animerandom":
        {
          ...
          db.data.users[m.sender].limit -= 1;
          let res = await fetch(
            "https://raw.githubusercontent.com/ArugaZ/grabbed-results/main/random/anime/random.txt",
          );
          ...
          await zyko.sendFile(
            ...
            {
              mentions: [m.sender],
              jpegThumbnail: await (await fetch(cita)).buffer(),
            },
          );
        }
        break;
...
`.trim();
      ...
      case "akira":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/akira.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
            m.chat,
            { image: { url: cita }, caption: command },
            { quoted: m },
          );
        }
        break;
      case "akiyama":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/akiyama.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "anna":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/ana.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "asuna":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/asuna.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "ayuzawa":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/ayuzawa.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "boruto":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/boruto.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "chitanda":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/chitanda.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "chitoge":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/chitoge.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "deidara":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/deidara.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "doraemon":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/doraemon.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "elaina":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/elaina.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "emilia":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/emilia.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "erza":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/erza.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "gremory":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/gremory.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "hestia":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/hestia.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "hinata":
        {
          ...
          db.data.users[m.sender].limit -= 1;
          let res = await fetch(
            `https://raw.githubusercontent.com/KazukoGans/database/main/anime/hinata.json`,
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
            m.chat,
      ...
      case "inori":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/inori.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "isuzu":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/isuzu.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "itachi":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/itachi.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "itori":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/itori.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "kaga":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/kaga.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "kagura":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/kagura.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "kakasih":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/kakasih.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "kaori":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/kaori.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "kosaki":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/kosaki.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "kotori":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/kotori.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "kuriyama":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/kuriyama.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "kuroha":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/kuroha.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "kurumi":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/kurumi.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "loli":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/loli.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "madara":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/madara.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "mikasa":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/mikasa.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "miku":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/miku.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "minato":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/minato.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "naruto":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/naruto.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "natsukawa":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/natsukawa.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "neko2":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/neko.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "nekohime":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/nekohime.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "nezuko":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/nezuko.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "nishimiya":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/nishimiya.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "onepiece":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/onepiece.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "pokemon":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/pokemon.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "rem":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/rem.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "rize":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/rize.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "sagiri":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/sagiri.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "sakura":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/sakura.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "sasuke":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/sasuke.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "shina":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/shina.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "shinka":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/shinka.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "shizuka":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/shizuka.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "shota":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/shota.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "tomori":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/tomori.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "toukachan":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/toukachan.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "tsunade":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/tsunade.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "yatogami":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/yatogami.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "yuki":
        {
          ...
          let res = await (
            await fetch(
              `https://raw.githubusercontent.com/KazukoGans/database/main/anime/yuki.json`,
            )
          ).json();
          let cita = res[Math.floor(Math.random() * res.length)];
          zyko.sendMessage(
      ...
      case "tomp3":
        {
          ...
          if (!/video/.test(mime) && !/audio/.test(mime))
            return reply(
              `Kirim/Reply Video/Audio Yang Ingin Dijadikan MP3 Dengan Caption ${prefix + command}`,
            );
          reply(mess.wait);
          let media = await zyko.downloadMediaMessage(qmsg);
          let audio = await toAudio(media, "mp4");
      ...
      case "toptt":
        {
          ...
          if (!/video/.test(mime) && !/audio/.test(mime))
            return reply(
              `Reply Video/Audio Yang Ingin Dijadikan VN Dengan Caption ${prefix + command}`,
            );
          reply(mess.wait);
          let media = await zyko.downloadMediaMessage(qmsg);
      ...
      case "setexif":
        {
          ...
          if (!isOwner) return reply(mess.owner);
          if (!text)
            return reply(`Contoh : ${prefix + command} packname|author`);
      ...
      case "smeme":
        ...
        if (/image/.test(mime)) {
          ...
          zyko.sendImageAsSticker(m.chat, kaytid, m, {
            packname: global.packname,
            author: global.author,
          });
        }
      ...
      case "swm":
        {
          ...
          if (!teks1)
            return reply(
              `Kirim/reply image/video dengan caption ${prefix + command} teks1|teks2`,
            );
          if (!teks2)
            return reply(
              `Kirim/reply image/video dengan caption ${prefix + command} teks1|teks2`,
      ...
      case "setppbot":
        {
          ...
          var medis = await zyko.downloadAndSaveMediaMessage(
            quoted,
            "ppbot.jpeg",
          );
          if (args[0] == `/full`) {
            ...
            var memeg = await zyko.updateProfilePicture(botNumber, {
              url: medis,
            });
            fs.unlinkSync(medis);
            reply(`Sukses`);
      ...
      case "carikawangc":
        {
          ...
          zyko.sendMessage(
            m.chat,
            { text: `Nih Kak ${phoneNumber.split("@")[0]}\n\n${waLink}` },
            m,
          );
        }
        break;
      ...
      case "editinfo":
        {
          ...
          if (args[0] === "open") {
            await zyko
              ...
              .then((res) => reply(`Sukses Membuka Edit Info Group 🕊️`))
              .catch((err) => reply(jsonformat(err)));
          } else if (args[0] === "close") {
            await zyko
              .groupSettingUpdate(m.chat, "locked")
              .then((res) => reply(`Sukses Menutup Edit Info Group 🕊️`))
              .catch((err) => reply(jsonformat(err)));
          } else {
      ...
      case "bcgroup":
        {
          ...
          m.reply(
            `Mengirim Broadcast Ke ${anu.length} Chat, Waktu Selesai ${anu.length * 0.5} detik`,
          );
          for (let i of anu) {
            zyko.sendMessage(
              i,
      ...
      case "bcchat":
        {
          ...
          let pesan = m.quoted && m.quoted.text ? m.quoted.text : text;
          m.reply(
            `Mengirim Broadcast Ke ${chats.length} Chat ${chats.length} `,
          );
          await delay(1500);
          for (let id of chats) {
            zyko.sendMessage(
      ...
      case "addlist":
        function _0x188a() {
          var _0x1826ea = [
            "includes",
            "existsSync",
            "\x20*Nama\x20Item@Item*\x0a\x0a_Contoh_\x0a\x0a",
            "List\x20respon\x20dengan\x20key\x20:\x20*",
            "4189977apUbUl",
            "1041220qBKFWy",
            "16931mlUIGL",
            "12EcEzCR",
            ...
            "462xxGRbE",
            "unlinkSync",
            "Sukses\x20set\x20list\x20message\x20dengan\x20key\x20:\x20*",
            "845pXIMgJ",
            "Sukses\x20Add\x20List\x20Dengan\x20Kunci\x20:\x20*",
            "74053nfNvGL",
        ...
        function _0x2161ca(_0x5b3072, _0x19f425) {
          return _0x21a8(_0x5b3072 - 0xfc, _0x19f425);
        }
        (function (_0x4f9e61, _0x3253e4) {
          var _0x26454b = _0x4f9e61();
          function _0xbfa347(_0x632218, _0x1ff859) {
            return _0x21a8(_0x632218 - 0x25e, _0x1ff859);
          }
          while (!![]) {
            try {
              var _0x127416 =
                (-parseInt(_0xbfa347(0x3a2, 0x397)) / 0x1) *
                  (-parseInt(_0xbfa347(0x3a8, 0x3a6)) / 0x2) +
                parseInt(_0xbfa347(0x3a6, 0x39d)) / 0x3 +
                (parseInt(_0xbfa347(0x3aa, 0x3ab)) / 0x4) *
                  (-parseInt(_0xbfa347(0x3b2, 0x3bd)) / 0x5) +
                (parseInt(_0xbfa347(0x3af, 0x3af)) / 0x6) *
                  (-parseInt(_0xbfa347(0x3b4, 0x3b3)) / 0x7) +
                (-parseInt(_0xbfa347(0x3a5, 0x3a3)) / 0x8) *
                  (-parseInt(_0xbfa347(0x3a7, 0x3b1)) / 0x9) +
                -parseInt(_0xbfa347(0x3bb, 0x3be)) / 0xa +
                (parseInt(_0xbfa347(0x3ba, 0x3c2)) / 0xb) *
                  (parseInt(_0xbfa347(0x3a3, 0x3b0)) / 0xc);
              if (_0x127416 === _0x3253e4) break;
              else _0x26454b["push"](_0x26454b["shift"]());
            } catch (_0x5baf36) {
              _0x26454b["push"](_0x26454b["shift"]());
            }
          }
        })(_0x188a, 0x7fa42);
        if (!isOwner) return reply(mess["owner"]);
        if (!m["isGroup"]) return reply(mess[_0x2161ca(0x249, 0x24d)]);
        var args1 = text["split"]("@")[0x0],
          args2 = text[_0x2161ca(0x24b, 0x246)]("@")[0x1];
        function _0x21a8(_0x474419, _0x112e86) {
          var _0x188aaa = _0x188a();
          return (
            (_0x21a8 = function (_0x21a8cf, _0x380815) {
              _0x21a8cf = _0x21a8cf - 0x144;
              var _0x10d027 = _0x188aaa[_0x21a8cf];
              return _0x10d027;
            }),
            _0x21a8(_0x474419, _0x112e86)
          );
        }
        if (!q[_0x2161ca(0x254, 0x25f)]("@"))
          return reply(
            "Gunakan\x20dengan\x20cara\x20" +
              (prefix + command[_0x2161ca(0x24a, 0x242)](0x0)) +
              _0x2161ca(0x256, 0x251) +
              (prefix + command[_0x2161ca(0x24a, 0x242)](0x0)) +
              _0x2161ca(0x242, 0x23e),
          );
        if (isAlreadyResponList(from, args1, db_respon_list))
          return reply(
            _0x2161ca(0x257, 0x24f) + args1 + _0x2161ca(0x247, 0x251),
          );
        if (/image/[_0x2161ca(0x24c, 0x256)](mime)) {
          (media = await zyko[_0x2161ca(0x253, 0x250)](quoted)),
            (mem = await TelegraPh(media)),
            addResponList(from, args1, args2, !![], "" + mem, db_respon_list),
            reply(_0x2161ca(0x24f, 0x250) + args1 + "*");
          if (fs[_0x2161ca(0x255, 0x252)](media))
            fs[_0x2161ca(0x24e, 0x259)](media);
        } else
          addResponList(from, args1, args2, ![], "-", db_respon_list),
            reply(_0x2161ca(0x251, 0x246) + args1 + "*");
        break;
      case "dellist":
        (function (_0x51864a, _0x2c0b46) {
          function _0x2e272c(_0x43be06, _0x950e3c) {
            return _0x30ef(_0x43be06 - 0x272, _0x950e3c);
          }
          var _0x2cbc58 = _0x51864a();
          while (!![]) {
            try {
              var _0x9c5001 =
                (-parseInt(_0x2e272c(0x332, 0x33a)) / 0x1) *
                  (-parseInt(_0x2e272c(0x333, 0x32f)) / 0x2) +
                -parseInt(_0x2e272c(0x33d, 0x33e)) / 0x3 +
                parseInt(_0x2e272c(0x33b, 0x343)) / 0x4 +
                parseInt(_0x2e272c(0x33c, 0x33f)) / 0x5 +
                (-parseInt(_0x2e272c(0x33a, 0x344)) / 0x6) *
                  (parseInt(_0x2e272c(0x32d, 0x333)) / 0x7) +
                -parseInt(_0x2e272c(0x335, 0x33f)) / 0x8 +
                (parseInt(_0x2e272c(0x331, 0x331)) / 0x9) *
                  (parseInt(_0x2e272c(0x334, 0x33d)) / 0xa);
              if (_0x9c5001 === _0x2c0b46) break;
              else _0x2cbc58["push"](_0x2cbc58["shift"]());
            } catch (_0xb343aa) {
              _0x2cbc58["push"](_0x2cbc58["shift"]());
            }
          }
        })(_0x3b5a, 0x1fe7c);
        if (!isOwner) return reply(mess[_0x37032d(-0x30e, -0x305)]);
        if (!m[_0x37032d(-0x31b, -0x319)])
          return reply(mess[_0x37032d(-0x316, -0x315)]);
        if (db_respon_list["length"] === 0x0)
          return reply(_0x37032d(-0x317, -0x317));
        function _0x3b5a() {
          var _0x3b9e46 = [
            "List\x20Item\x20dengan\x20Nama\x20*",
            ...
            "769952iLWfGq",
            "slice",
            "\x20*Nama\x20Item*\x0a\x0a_Contoh_\x0a\x0a",
            "owner",
            "\x20namalist",
            "498qAfPhQ",
            "69712BQureP",
            "951145CngebQ",
            "750132KGPlxx",
            "Sukses\x20delete\x20list\x20message\x20dengan\x20key\x20*",
            "isGroup",
            "*\x20tidak\x20ada\x20di\x20database!",
            "12775EGUQUY",
          ];
          _0x3b5a = function () {
            return _0x3b9e46;
        ...
        if (!q)
          return reply(
            "Gunakan\x20dengan\x20cara\x20" +
              command[_0x37032d(-0x310, -0x313)](0x1) +
              _0x37032d(-0x30f, -0x30f) +
              command[_0x37032d(-0x310, -0x30d)](0x1) +
              _0x37032d(-0x30d, -0x307),
          );
        if (!isAlreadyResponList(from, q, db_respon_list))
          return reply(
            _0x37032d(-0x318, -0x31d) + q + _0x37032d(-0x31a, -0x314),
          );
        delResponList(from, q, db_respon_list);
        function _0x30ef(_0x1e9199, _0x3caf82) {
          var _0x3b5a10 = _0x3b5a();
          return (
            (_0x30ef = function (_0x30ef21, _0x315a6f) {
              _0x30ef21 = _0x30ef21 - 0xb9;
              var _0x3e9943 = _0x3b5a10[_0x30ef21];
              return _0x3e9943;
            }),
            _0x30ef(_0x1e9199, _0x3caf82)
          );
        }
        function _0x37032d(_0x1ee0b7, _0x6aa986) {
          return _0x30ef(_0x1ee0b7 - -0x3d4, _0x6aa986);
        }
        reply(_0x37032d(-0x308, -0x2ff) + q + "*");
        break;
      case "updatelist":
        (function (_0x1fec75, _0x75f63a) {
          var _0x10e016 = _0x1fec75();
          function _0x4b8f68(_0x903b16, _0x3ce9d4) {
            return _0x2613(_0x903b16 - 0x1b1, _0x3ce9d4);
          }
          while (!![]) {
            try {
              var _0x43cff3 =
                -parseInt(_0x4b8f68(0x21f, 0x227)) / 0x1 +
                (-parseInt(_0x4b8f68(0x225, 0x225)) / 0x2) *
                  (-parseInt(_0x4b8f68(0x22e, 0x237)) / 0x3) +
                (parseInt(_0x4b8f68(0x230, 0x225)) / 0x4) *
                  (parseInt(_0x4b8f68(0x231, 0x233)) / 0x5) +
                parseInt(_0x4b8f68(0x224, 0x222)) / 0x6 +
                parseInt(_0x4b8f68(0x22f, 0x236)) / 0x7 +
                (parseInt(_0x4b8f68(0x223, 0x229)) / 0x8) *
                  (-parseInt(_0x4b8f68(0x221, 0x22d)) / 0x9) +
                -parseInt(_0x4b8f68(0x226, 0x21c)) / 0xa;
              if (_0x43cff3 === _0x75f63a) break;
              else _0x10e016["push"](_0x10e016["shift"]());
            } catch (_0x184e9a) {
              _0x10e016["push"](_0x10e016["shift"]());
            }
          }
        })(_0x56ea, 0x45ac7);
        if (!isOwner) return reply(mess[_0x6ca23e(0x279, 0x273)]);
        if (!m["isGroup"]) return reply(mess[_0x6ca23e(0x27f, 0x27d)]);
        var args1 = q[_0x6ca23e(0x272, 0x27d)]("@")[0x0],
          args2 = q[_0x6ca23e(0x272, 0x277)]("@")[0x1];
        if (!q[_0x6ca23e(0x286, 0x27f)]("@"))
          return reply(
            "Gunakan\x20dengan\x20cara\x20" +
              command["slice"](0x1) +
              _0x6ca23e(0x27d, 0x276) +
              command[_0x6ca23e(0x287, 0x28b)](0x1) +
              "\x20namalist@List",
          );
        function _0x6ca23e(_0x4262c5, _0x4b1e2f) {
          return _0x2613(_0x4262c5 - 0x203, _0x4b1e2f);
        }
        if (!isAlreadyResponListGroup(from, db_respon_list))
          return reply(
            _0x6ca23e(0x274, 0x268) + args1 + _0x6ca23e(0x27e, 0x27a),
          );
        if (/image/[_0x6ca23e(0x288, 0x280)](mime)) {
          (media = await zyko[_0x6ca23e(0x284, 0x28f)](quoted)),
            (mem = await TelegraPh(media)),
            updateResponList(
              from,
              args1,
            ...
            reply(_0x6ca23e(0x27c, 0x275) + args1 + "*");
          if (fs[_0x6ca23e(0x27b, 0x272)](media))
            fs[_0x6ca23e(0x27a, 0x270)](media);
        } else
          updateResponList(from, args1, args2, ![], "-", db_respon_list),
            reply(_0x6ca23e(0x285, 0x27a) + args1 + "*");
        function _0x2613(_0x2d2715, _0x17bcd3) {
          var _0x56ea90 = _0x56ea();
          return (
            (_0x2613 = function (_0x2613ae, _0x3da915) {
              _0x2613ae = _0x2613ae - 0x6e;
              var _0x16e5d5 = _0x56ea90[_0x2613ae];
              return _0x16e5d5;
            }),
            _0x2613(_0x2d2715, _0x17bcd3)
          );
        }
        function _0x56ea() {
          var _0x5706ae = [
            ...
            "35yeaLpW",
            "downloadAndSaveMediaMessage",
            "Sukses\x20update\x20respon\x20list\x20dengan\x20key\x20*",
            "includes",
            "slice",
            "test",
            "393977DHGbAi",
            ...
            "unlinkSync",
            "existsSync",
            "Sukses\x20update\x20list\x20message\x20dengan\x20key\x20:\x20*",
            "\x20*Nama\x20Item@Item*\x0a\x0a_Contoh_\x0a\x0a",
            "*\x20belum\x20terdaftar\x20di\x20group\x20ini",
            "group",
          ];
          _0x56ea = function () {
            return _0x5706ae;
      ...
      case "list":
        {
          ...
          if (db_respon_list.length === 0)
            return reply(`*Belum ada list message di Group ini*`);
...
`;
      ...
      case "orkut":
        zyko.sendMessage(
          from,
          {
            image: { url: "https://telegra.ph/file/0ebc4a733052351b2b8e7.jpg" },
            caption:
              "Silahkan Scan QR OrderKuota Diatas Jangan Lupa Kirim Bukti Transaksinya Ya",
          },
          { quoted: zyko.chat },
        );
        break;
      ...
      case "dana":
        zyko.sendMessage(
          from,
          {
            image: { url: "https://telegra.ph/file/51b29b4f9bdacb9692d47.png" },
            caption:
              "Silahkan Scan Code Qr Diatas Jangan Lupa Kirim Bukti Transaksinya Ke wa.me/6285175463174",
          },
          { quoted: zyko.chat },
        );
        break;
      ...
      case "gojek":
        zyko.sendMessage(
          from,
          {
            image: { url: "https://telegra.ph/file/3a146c6a231be82220606.jpg" },
            caption:
              "Silahkan Scan Code Qr Diatas Jangan Lupa Kirim Bukti Transaksinya Ke wa.me/6285175463174",
          },
          { quoted: zyko.chat },
      ...
      case "buyer":
        {
          ...
          let ngen = ["|"];
          if (args[0].includes(ngen))
            return reply("Replace Symbol | So spaces");
...
`;
      ...
      case "1gb":
        {
          if (!isSeler && !isOwner)
            return reply(`Fitur Ini Khusus Seller Dan Owner`);
          let t = text.split(",");
          if (t.length < 2)
...
${prefix + command} email,username,name,number/tag`);
          ...
          if (data.errors)
            return reply(JSON.stringify(data.errors[0], null, 2));
          let user = data.attributes;
          let p = await zyko.sendMessage(m.chat, {
            text: `
      ...
      case "restartsrv":
        {
          ...
          if (!srv) return reply("ID nya mana?");
          let f = await fetch(
            domain + "/api/client/servers/" + srv + "/power",
            {
              method: "POST",
              headers: {
                Accept: "application/json",
